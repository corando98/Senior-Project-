...
print(
    "/!\ Send message through MQTT to topic detection, choose face or litter as message"
)
server = "localhost"
path = "model"
...
def runModel():
    ...
    while loop and vs.isOpened() and (message.find("litter") != -1):
        try:
            ...
            objs = detect.get_objects(inferenceEngine, 0.2)
...
def runModelFace():
    ...
    # Sweet framerate spots: 40 (no resulution set)
    # More info on framerate / resultion https://picamera.readthedocs.io/en/latest/fov.html#sensor-modes
    ...
    while loop and (message.find("face") != -1):
        try:
            start = time.perf_counter()
            # Reading frame
            screenshot = vs.read()
